...
#include "EZapArch.h"
#include "EZapArchMachineFunctionInfo.h"
...
static const MCPhysReg ArgGPRs[] = {EZapArch::R9, EZapArch::R10, EZapArch::R11, EZapArch::R12};

void EZapArchTargetLowering::ReplaceNodeResults(SDNode *N,
...
EZapArchTargetLowering::EZapArchTargetLowering(const TargetMachine &TM,
  ...
  setStackPointerRegisterToSaveRestore(EZapArch::R1);
...
#include "EZapArchGenCallingConv.inc"
...
SDValue EZapArchTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_EZapArch);
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, EZapArch::R1, PtrVT);
      SDValue Address =
  ...
  Chain = DAG.getNode(EZapArchISD::CALL, DL, NodeTys, Ops);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_EZapArch);
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
                                   ...
                                   const EZapArchSubtarget &Subtarget) {
...
static SDValue unpackFromRegLoc(SelectionDAG &DAG, SDValue Chain,
                                const CCValAssign &VA, const SDLoc &DL,
                                const EZapArchTargetLowering &TLI) {
  MachineFunction &MF = DAG.getMachineFunction();
...
/// EZapArch formal arguments implementation
SDValue EZapArchTargetLowering::LowerFormalArguments(
  ...
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_EZapArch);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &EZapArch::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    EZapArchFunctionInfo *UFI = MF.getInfo<EZapArchFunctionInfo>();
...
bool EZapArchTargetLowering::CanLowerReturn(
  ...
  if (!CCInfo.CheckReturn(Outs, RetCC_EZapArch))
    return false;
...
SDValue
EZapArchTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                    bool IsVarArg,
  ...
  const EZapArchSubtarget &STI = MF.getSubtarget<EZapArchSubtarget>();
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_EZapArch);
  ...
  return DAG.getNode(EZapArchISD::RET, DL, MVT::Other, RetOps);
}
...
SDValue EZapArchTargetLowering::PerformDAGCombine(SDNode *N,
...
bool EZapArchTargetLowering::isLegalAddressingMode(const DataLayout &DL,
